...
export const adminService = {
  ...
  getUsers: async (params?: Record<string, any>) => {
    const response = await axios.get<{ users: User[]; total: number; }>(`${API_URL}/admin/users`, {
      params,
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token') || ''}`
      }
  ...
  getStores: async (params?: Record<string, any>) => {
    const response = await axios.get<{ stores: Store[]; total: number; }>(`${API_URL}/admin/stores`, {
      params,
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token') || ''}`
      }
